...
function App() {
  ...
  const activeFile = useStore(state => state.files?.activeFile ?? null);
  const language = useStore(state => state.settings?.language ?? 'javascript');
  const models = useStore(state => state.ai?.models ?? []);
  const isConnected = useStore(state => state.ai?.isConnected ?? false);
  const showSettings = useStore(state => state.ui?.showSettings ?? false);
  const projectPath = useStore(state => state.project?.projectPath ?? null);
  const deliberationMessages = useStore(state => state.ai?.deliberationMessages ?? []);
  const activeTab = useStore(state => state.ui?.activeTab ?? 'editor');
  const fileVersions = useStore(state => state.files?.fileVersions ?? {});
  const outputModal = useStore(state => state.ui?.outputModal ?? { isOpen: false, title: '', message: '', isError: false });
  const showShortcuts = useStore(state => state.ui?.showShortcuts ?? false);
  const dirtyFiles = useStore(state => state.files?.dirtyFiles ?? {});
  const lastSavedTimes = useStore(state => state.files?.lastSavedTimes ?? {});
  const saveDialog = useStore(state => state.ui?.saveDialog ?? { isOpen: false, fileName: '', multiple: false, unsavedFiles: [] });
  ...
  const activeTerminalId = useStore(state => state.ui?.activeTerminalId ?? null);
  const showTerminal = useStore(state => state.ui?.showTerminal ?? false);
  const showSnippetPanel = useStore(state => state.ui?.showSnippetPanel ?? false);
  const showQualityMetrics = useStore(state => state.ui?.showQualityPanel ?? false);
  const showRubricEditor = useStore(state => state.ui?.showRubricEditor ?? false);
  const evaluationData = useStore(state => state.ui?.evaluationData ?? null);
  ...
  const setLanguageAction = setLanguage; // Alias
  const setModels = useStore(state => state.setModels);
  const setModelsAction = setModels; // Alias
  ...
  const setProjectPathAction = setProjectPath; // Alias
  ...
  const setSaveDialog = useStore(state => state.setSaveDialog);
  const setSaveDialogAction = setSaveDialog; // Alias
  const setRecentFiles = useStore(state => state.setRecentFiles);
  const setRecentFilesAction = setRecentFiles; // Alias
  const setLspServers = useStore(state => state.setLspServers);
  const setLspServersAction = setLspServers; // Alias
  ...
  const setEvaluationData = useStore(state => state.setEvaluationData);
  const setEvaluationDataAction = setEvaluationData; // Alias
  const setEvaluationHistory = useStore(state => state.setEvaluationHistory);
  const setEvaluationHistoryAction = setEvaluationHistory; // Alias
  ...
  const startResizing = (direction) => (mouseDownEvent) => {
    ...
    const startWidth = direction === 'left' ? sidebarWidth : aiPanelWidth;
  ...
  const handleDebugStepCompleted = (data) => {
    ...
    if (typeof loadDebugData === 'function') {
      // Use data.sessionId if available, or rely on passed data
      loadDebugData({ ...data, currentPosition: data.position });
    }
  };

  const handleDebugBreakpointSet = (data) => {
    logger.debug('Breakpoint set', data);
    if (typeof loadDebugBreakpoints === 'function') loadDebugBreakpoints();
  };
  ...
  // Unsaved changes listener effect
  useEffect(() => {
  ...
  }, [files, activeFile]); // Re-bind when potential closure dependencies change
  ...
  const handleFileSelect = async (filePath, content, isEditor = true) => {
    logger.info('File selected', { filePath });
    setActiveFileAction(filePath);

    // Lazy load content if not already available
    let fileContent = content;
    if (fileContent === null && window.electronAPI) {
      // Show loading or just wait
      const result = await window.electronAPI.readFile(filePath);
      if (result.success) {
        fileContent = result.content;
        // Update files store with the new content, preserving existing files
        setFileContentsAction({ ...files, [filePath]: fileContent });
      } else {
    ...
    // Update active tab based on where selection happened
    if (isEditor) {
      setActiveTabAction('editor');
    }
  ...
  const handleSaveAs = async () => {
    ...
    if (result.success && result.path) {
      ...
      try {
        const resultSave = await window.electronAPI.saveFile(result.path, content);
    ...
    try {
      ...
      if (result.success && result.path) {
        ...
        if (result.files) {
          ...
          if (firstFile && result.files[firstFile]) {
  ...
  const handleSaveProject = async () => {
    if (activeFile) {
      await handleFileSave(activeFile, files[activeFile]);
    }
  };
  ...
  const handleNewFile = () => {
    ...
    createFileAction(name, '');
    setActiveFileAction(name);
  };

  const handleRunCode = async () => {
    if (!activeFile) return;
    const code = files[activeFile];
    setOutputModalAction({ isOpen: true, title: 'Running Code...', message: 'Executing...', isError: false });
    try {
      const result = await window.electronAPI.runCode(activeFile, language, code);
      if (result.success) {
        setOutputModalAction({ isOpen: true, title: 'Execution Result', message: result.output, isError: false });
      } else {
        setOutputModalAction({ isOpen: true, title: 'Execution Failed', message: result.error, isError: true });
      }
    } catch (error) {
      setOutputModalAction({ isOpen: true, title: 'Execution Error', message: error.message, isError: true });
    }
  };
  ...
  return (
    <div className="app-container">
      <div className="workspace" style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        <div className="sidebar" style={{ width: sidebarWidth, flexShrink: 0, overflow: 'hidden' }}>
          <FileExplorer
            files={files}
        ...
        <div className="resizer" onMouseDown={startResizing('left')} />
        <div className="main-content">
          ...
          <div className="resizer" onMouseDown={startResizing('right')} />
          <div className="ai-panel-wrapper" style={{ width: aiPanelWidth, flexShrink: 0 }}>
            <AIPanel
              activeFile={activeFile}
      ...
      <StatusBar
        ...
        isConnected={isConnected}
        activeFile={activeFile}
        isDirty={activeFile && dirtyFiles ? dirtyFiles[activeFile] : false}
        lastSaved={activeFile && lastSavedTimes ? lastSavedTimes[activeFile] : null}
        lspStatus={language && lspServers && lspServers[language] ? lspServers[language].status : 'disconnected'}
